...
export function sohScenarios(points: TelemetryPoint[], est: SohEstimate, refs: SohReferences): SohScenario[] {
  ...
  if (rr) out.push({ id: "rated_range", label: "Rated range (EPA)", sohPercent: rr.sohPercent, sigmaPercent: rr.sigmaPercent, value: rr.fullValue, unit: "mi", reference: rr.reference, formula: "RatedRange at completed full charges ÷ 272 mi EPA", note: "Only method with a verified reference." });
  ...
  if (refs.ratedRangeMi && events.length) {
    ...
    out.push({ id: "optimistic", label: "Best full charge", sohPercent: r2(100 * best / refs.ratedRangeMi), value: r2(best), unit: "mi", reference: refs.ratedRangeMi, formula: "highest qualified full-charge rated range ÷ 272", note: "Upper bound: the BMS's most generous full-pack estimate." });
